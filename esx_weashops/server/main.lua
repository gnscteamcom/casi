...
AddEventHandler('esx_weashop:buyCaricatore', function(zone)
  ...
  if zone=="BlackWeashop" then
    if xPlayer.get('money') >= 500 then
		...
		TriggerClientEvent('esx:showNotification', _source, '¡Ya tiene el máximo de cargadores transportables!')
		else
    ...
    xPlayer.addInventoryItem('clip', 1)
    TriggerClientEvent('esx:showNotification', _source, 'compraste un cargador')
	end
  else
    TriggerClientEvent('esx:showNotification', _source, _U('not_enough'))
  end
